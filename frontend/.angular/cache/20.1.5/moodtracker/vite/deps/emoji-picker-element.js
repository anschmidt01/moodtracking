...
function getETag(dataSource) {
  return __async(this, null, function* () {
    const response = yield fetch(dataSource, { method: "HEAD" });
...
function doCheckForUpdates(db, dataSource) {
  return __async(this, null, function* () {
    let emojiData;
...
function checkForUpdates(db, dataSource) {
  return __async(this, null, function* () {
    try {
      yield doCheckForUpdates(db, dataSource);
    } catch (err) {
...
function createRoot(shadowRoot, props) {
  ...
  createEffect(() => {
    function updateFavorites() {
      return __async(this, null, function* () {
        ...
        const favorites = yield summarizeEmojis(uniqBy2([
          ...dbFavorites,
  ...
  function clickEmoji(unicodeOrName) {
    return __async(this, null, function* () {
      ...
      }, skinTonedUnicode && { unicode: skinTonedUnicode }), emojiSummary.name && { name: emojiSummary.name }));
    });
  }
  ...
  function onSkinToneOptionsKeydown(event) {
    ...
    const changeActiveSkinTone = (nextSkinTone) => __async(null, null, function* () {
      halt(event);
